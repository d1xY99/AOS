...
then
    if [[ $(qemu-system-aarch64 -machine help | grep raspi3b | wc -l) != "0" ]]; then
        qemu-system-aarch64 -M raspi3b -cpu cortex-a53 -m 1024 -drive file=AOS-flat.vmdk,if=sd,format=raw -no-reboot -kernel kernel.x -serial file:/tmp/out.log -d guest_errors,unimp -monitor pipe:/tmp/qemu -nographic -display none  > /dev/null 2> /dev/null  &
        sleep 10
    else
...
HAS_SHELL=$(grep -c "AOS: />" /tmp/out.log)
HAS_HELP=$(grep -c "Command Help" /tmp/out.log)

if [[ $HAS_SHELL > 0 ]] && [[ $HAS_HELP > 0 ]]; 
then
    echo "AOS boots and has a (working) shell"
    exit 0
fi

if [[ $HAS_SHELL > 0 ]] && [[ $HAS_HELP == 0 ]];
then
    echo "AOS boots but has no working shell"
    exit 0
fi

if [[ $HAS_SHELL == 0 ]];
then
    echo "AOS does not boot to a shell"
    exit 1
fi
