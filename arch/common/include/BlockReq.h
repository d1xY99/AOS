...
{
  protected:
    friend class BlockDev;
    friend class ATADriver;
    friend class MMCDriver;
    friend class BlockDevTable;
     ...
     *
     */
    BlockReq( uint32 dev_id, BD_CMD cmd, uint32 start_block = 0, uint32 num_block = 0, void * buffer = 0 ) 
    {
    ...
    BlockReq *getNextRequest(){ return next_request_; };
    ...
    void setNextRequest( BlockReq *next ){ next_request_=next; };
