...
Result HcdPrepareChannel(struct UsbDevice *device, u8 channel,
  ...
  if (channel > Core->Hardware.HostChannelCount) {
    LOGF("HCD: Pipe %d is not available on this host.\n", channel);
    return ErrorArgument;
  }

  // Clear all existing interrupts.
  SetReg(&Host->Pipe[channel].Interrupt);
  WriteThroughReg(&Host->Pipe[channel].Interrupt);

  // Program the channel.
  ClearReg(&Host->Pipe[channel].Characteristic);
  Host->Pipe[channel].Characteristic.DeviceAddress = pipe->Device;
  Host->Pipe[channel].Characteristic.EndPointNumber = pipe->EndPoint;
  Host->Pipe[channel].Characteristic.EndPointDirection = pipe->Direction;
  Host->Pipe[channel].Characteristic.LowSpeed = pipe->Speed == Low ? true : false;
  Host->Pipe[channel].Characteristic.Type = pipe->Type;
  Host->Pipe[channel].Characteristic.MaximumPacketSize = SizeToNumber(pipe->MaxSize);
  Host->Pipe[channel].Characteristic.Enable = false;
  Host->Pipe[channel].Characteristic.Disable = false;
  WriteThroughReg(&Host->Pipe[channel].Characteristic);

  // Clear split control.
  ClearReg(&Host->Pipe[channel].SplitControl);
  if (pipe->Speed != High) {
    Host->Pipe[channel].SplitControl.SplitEnable = true;
    Host->Pipe[channel].SplitControl.HubAddress = device->Parent->Number;
    Host->Pipe[channel].SplitControl.PortAddress = device->PortNumber;     
  }
  WriteThroughReg(&Host->Pipe[channel].SplitControl);

  ClearReg(&Host->Pipe[channel].TransferSize);
  Host->Pipe[channel].TransferSize.TransferSize = length;
  if (pipe->Speed == Low)
    Host->Pipe[channel].TransferSize.PacketCount = (length + 7) / 8;
  else
    Host->Pipe[channel].TransferSize.PacketCount = (length + Host->Pipe[channel].Characteristic.MaximumPacketSize -  1) / Host->Pipe[channel].Characteristic.MaximumPacketSize;
  if (Host->Pipe[channel].TransferSize.PacketCount == 0)
    Host->Pipe[channel].TransferSize.PacketCount = 1;
  Host->Pipe[channel].TransferSize.PacketId = type;
  WriteThroughReg(&Host->Pipe[channel].TransferSize);
  
  return OK;
}

void HcdTransmitChannel(u8 channel, void* buffer) { 
  ReadBackReg(&Host->Pipe[channel].SplitControl);
  Host->Pipe[channel].SplitControl.CompleteSplit = false;
  WriteThroughReg(&Host->Pipe[channel].SplitControl);

  if (((u32)buffer & 3) != 0)
    LOG_DEBUGF("HCD: Transfer buffer %p is not DWORD aligned. Ignored, but dangerous.\n", buffer);
  Host->Pipe[channel].DmaAddress = buffer;
  WriteThroughReg(&Host->Pipe[channel].DmaAddress);

  ReadBackReg(&Host->Pipe[channel].Characteristic);
  Host->Pipe[channel].Characteristic.PacketsPerFrame = 1;
  Host->Pipe[channel].Characteristic.Enable = true;
  Host->Pipe[channel].Characteristic.Disable = false;
  WriteThroughReg(&Host->Pipe[channel].Characteristic);  
}
...
Result HcdChannelSendWaitOne(struct UsbDevice *device, 
  ...
  for (globalTries = 0, actualTries = 0; globalTries < 3 && actualTries < 10; globalTries++, actualTries++) {
    SetReg(&Host->Pipe[channel].Interrupt);
    WriteThroughReg(&Host->Pipe[channel].Interrupt);
    ReadBackReg(&Host->Pipe[channel].TransferSize);
    ReadBackReg(&Host->Pipe[channel].SplitControl);
    ...
    do {
      ...
      ReadBackReg(&Host->Pipe[channel].Interrupt);
      if (!Host->Pipe[channel].Interrupt.Halt) MicroDelay(10);
      else break;
    } while (true);
    ReadBackReg(&Host->Pipe[channel].TransferSize);
    
    if (Host->Pipe[channel].SplitControl.SplitEnable) {
      if (Host->Pipe[channel].Interrupt.Acknowledgement) {
        for (tries = 0; tries < 3; tries++) {
          SetReg(&Host->Pipe[channel].Interrupt);
          WriteThroughReg(&Host->Pipe[channel].Interrupt);

          ReadBackReg(&Host->Pipe[channel].SplitControl);
          Host->Pipe[channel].SplitControl.CompleteSplit = true;
          WriteThroughReg(&Host->Pipe[channel].SplitControl);
          ...
          do {
            ...
            ReadBackReg(&Host->Pipe[channel].Interrupt);
            if (!Host->Pipe[channel].Interrupt.Halt) MicroDelay(100);
            else break;
          } while (true);
          if (!Host->Pipe[channel].Interrupt.NotYet) break;
        }

        if (tries == 3) {
          MicroDelay(25000);
          continue;
        } else if (Host->Pipe[channel].Interrupt.NegativeAcknowledgement) {
          globalTries--;
          MicroDelay(25000);
          continue;
        } else if (Host->Pipe[channel].Interrupt.TransactionError) {
          MicroDelay(25000);
          continue;
        }
  
        if ((result = HcdChannelInterruptToError(device, Host->Pipe[channel].Interrupt, false)) != OK) {
      ...
      if ((result = HcdChannelInterruptToError(device, Host->Pipe[channel].Interrupt, !Host->Pipe[channel].SplitControl.SplitEnable)) != OK) {
        LOG_DEBUGF("HCD: Control message to %#x: %02x%02x%02x%02x %02x%02x%02x%02x.\n", *(u32*)pipe, 
          ((u8*)request)[0], ((u8*)request)[1], ((u8*)request)[2], ((u8*)request)[3],
          ((u8*)request)[4], ((u8*)request)[5], ((u8*)request)[6], ((u8*)request)[7]);
...
retry:
  ...
  transfer = 0;
  do {
    packets = Host->Pipe[channel].TransferSize.PacketCount;
    ...
    ReadBackReg(&Host->Pipe[channel].TransferSize);    
    transfer = bufferLength - Host->Pipe[channel].TransferSize.TransferSize;
    if (packets == Host->Pipe[channel].TransferSize.PacketCount) break;
  } while (Host->Pipe[channel].TransferSize.PacketCount > 0);

  if (packets == Host->Pipe[channel].TransferSize.PacketCount) {
    device->Error = ConnectionError;
...
Result HcdSumbitControlMessage(struct UsbDevice *device, 
  ...
  if (buffer != NULL) {
    ...
    ReadBackReg(&Host->Pipe[0].TransferSize);
    if (pipe.Direction == In) {
      if (Host->Pipe[0].TransferSize.TransferSize <= bufferLength)
        device->LastTransfer = bufferLength - Host->Pipe[0].TransferSize.TransferSize;
      else{
        LOG_DEBUGF("HCD: Weird transfer.. %d/%d bytes received.\n", Host->Pipe[0].TransferSize.TransferSize, bufferLength);
  ...
  ReadBackReg(&Host->Pipe[0].TransferSize);
  if (Host->Pipe[0].TransferSize.TransferSize != 0)
    LOG_DEBUGF("HCD: Warning non zero status transfer! %d.\n", Host->Pipe[0].TransferSize.TransferSize);
...
Result HcdStart() { 
  ...
  if (!Host->Config.EnableDmaDescriptor) {
    for (u32 channel = 0; channel < Core->Hardware.HostChannelCount; channel++) {
      ReadBackReg(&Host->Pipe[channel].Characteristic);
      Host->Pipe[channel].Characteristic.Enable = false;
      Host->Pipe[channel].Characteristic.Disable = true;
      Host->Pipe[channel].Characteristic.EndPointDirection = In;
      WriteThroughReg(&Host->Pipe[channel].Characteristic);
      timeout = 0;
    }
    ...
    for (u32 channel = 0; channel < Core->Hardware.HostChannelCount; channel++) {
      ...
      } while (Host->Pipe[channel].Characteristic.Enable);
    }
  }
