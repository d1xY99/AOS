...
{
  page_dir_page_ = FrameAlloc::instance()->allocPPN(PD_SIZE);
...
void ArchMemory::checkAndRemovePT(uint32 pde_vpn)
{
  ...
  for (size_t i = 0; i < 4; ++i)
  {
    ...
    if (aostl::find(pt_ppns_.begin(), pt_ppns_.end(), pt_slot) == pt_ppns_.end())
      return;
  }

  FrameAlloc::instance()->freePPN(pt_ppn - PHYS_OFFSET_4K);

  for (size_t i = 0; i < 4; ++i)
  {
    uint32 pt_slot = (pt_ppn - PHYS_OFFSET_4K) * 4 + i;
    pt_ppns_.erase(aostl::find(pt_ppns_.begin(), pt_ppns_.end(), pt_slot));
  }
}
...
void ArchMemory::unmapPage(uint32 virtual_page)
{
  ...
  FrameAlloc::instance()->freePPN(pte_base[pte_vpn].page_ppn - PHYS_OFFSET_4K);
...
void ArchMemory::insertPT(uint32 pde_vpn)
{
  ...
  {
    physical_page_table_page = FrameAlloc::instance()->allocPPN();
    offset = 0;
...
bool ArchMemory::mapPage(uint32 virtual_page, uint32 physical_page, uint32 user_access,
                         uint32 writeable, bool /*executable*/)
{
  PageDirEntry *page_directory = (PageDirEntry *) getIdentAddressOfPPN(page_dir_page_);
  uint32 pde_vpn = virtual_page / PAGE_TABLE_ENTRIES;
  uint32 pte_vpn = virtual_page % PAGE_TABLE_ENTRIES;
  ...
  if (pte_base[pte_vpn].size == PDE_SIZE_NONE)
  {
    ...
    if (user_access)
      pte_base[pte_vpn].permissions = writeable ? PAGE_PERMISSION_WRITE : PAGE_PERMISSION_READ;
    else
...
ArchMemory::~ArchMemory()
{
  ...
  for (uint32 pde_vpn = 8; pde_vpn < PAGE_DIR_ENTRIES / 2; ++pde_vpn)
  {
    ...
    if (page_directory[pde_vpn].pt.size == PDE_SIZE_PT)
    {
      ...
      for (size_t i = 0; i < 4; ++i)
      {
        auto it = aostl::find(pt_ppns_.begin(), pt_ppns_.end(), page_directory[pde_vpn].pt.pt_ppn * 4 + i);
        if (it == pt_ppns_.end())
          free_pt_page = false;
        else
          pt_ppns_.erase(it);
      ...
      if (free_pt_page)
        FrameAlloc::instance()->freePPN(page_directory[pde_vpn].pt.pt_ppn - PHYS_OFFSET_4K);
    }
  }
  FrameAlloc::instance()->freePPN(page_dir_page_, PD_SIZE);
}
