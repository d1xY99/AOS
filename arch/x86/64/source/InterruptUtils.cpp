...
#define FLAG_PF_PRESENT                                                        \
  0x01 // =0: pt/page not present
...
#define FLAG_PF_USER                                                           \
  0x04 // =0: supervisormode (CPL < 3)
       // =1: usermode (CPL == 3)

#define FLAG_PF_RSVD                                                           \
  0x08 // =0: not a reserved bit
       // =1: a reserved bit

#define FLAG_PF_INSTR_FETCH                                                    \
  0x10 // =0: not an instruction fetch
       // =1: an instruction fetch (need PAE for that)

struct GateDesc {
  uint16 offset_ld_lw
      : 16; // low word / low dword of handler entry point's address
  ...
  uint16 offset_ld_hw
      : 16; // high word / low dword of handler entry point's address
...
void InterruptUtils::initialise() {
  ...
  size_t dummy_handler_sled_size =
      (((size_t)arch_dummyHandlerMiddle) - (size_t)arch_dummyHandler);
  assert((dummy_handler_sled_size % 128) == 0 &&
         "cannot handle weird padding in the kernel binary");
  ...
  for (uint32 i = 0; i < num_handlers; ++i) {
    while (handlers[j].number < i && handlers[j].offset != 0)
      ++j;
    interrupt_gates[i].offset_ld_lw = LO_WORD(LO_DWORD(
        (handlers[j].number == i && handlers[j].offset != 0)
            ? (size_t)handlers[j].offset
            : (((size_t)arch_dummyHandler) + i * dummy_handler_sled_size)));
    interrupt_gates[i].offset_ld_hw = HI_WORD(LO_DWORD(
        (handlers[j].number == i && handlers[j].offset != 0)
            ? (size_t)handlers[j].offset
            : (((size_t)arch_dummyHandler) + i * dummy_handler_sled_size)));
    interrupt_gates[i].offset_hd = HI_DWORD(
        (handlers[j].number == i && handlers[j].offset != 0)
            ? (size_t)handlers[j].offset
            : (((size_t)arch_dummyHandler) + i * dummy_handler_sled_size));
    interrupt_gates[i].ist = 0; // we could provide up to 7 different indices
    ...
    interrupt_gates[i].dpl =
        ((i == SYSCALL_INTERRUPT && handlers[j].number == i)
             ? DPL_USER_SPACE
             : DPL_KERNEL_SPACE);
    debug(A_INTERRUPTS,
          "%x -- offset = %p, offset_ld_lw = %x, offset_ld_hw = %x, offset_hd "
          "= %x, ist = %x, present = %x, segment_selector = %x, type = %x, dpl "
          "= %x\n",
          i, handlers[i].offset, interrupt_gates[i].offset_ld_lw,
          interrupt_gates[i].offset_ld_hw, interrupt_gates[i].offset_hd,
          interrupt_gates[i].ist, interrupt_gates[i].present,
          interrupt_gates[i].segment_selector, interrupt_gates[i].type,
          interrupt_gates[i].dpl);
  }
  IDTR idtr;
...
void InterruptUtils::countPageFault(uint64 address) {
  ...
  if (pf_address_counter >= 10) {
    kprintfd("same pagefault from the same thread for 10 times in a row. most "
             "likely you have an error in your code\n");
    asm("hlt");
  }
}

extern AOSDebugInfo const *kernel_debug_info;
...
extern "C" void irqHandler_0() {
  ...
  Scheduler::instance()->calculateCPUFrequency();
...
extern "C" void errorHandler(size_t num, size_t eip, size_t cs,
                             size_t spurious);
extern "C" void arch_pageFaultHandler();
extern "C" void pageFaultHandler(uint64 address, uint64 error) {
  auto &regs =
      *(currentThread->switch_to_userspace_ ? currentThread->user_registers_
                                            : currentThread->kernel_registers_);

  if (address >= USER_BREAK &&
      address <
          KERNEL_START) { // dirty hack due to qemu invoking the pf handler when
    // accessing non canonical addresses
  ...
  assert((regs.rflags & (1ULL << 9)) &&
         "PF with interrupts disabled. PF handler will enable interrupts soon. "
         "Better panic now");

  FaultDispatch::enterPageFault(
      address, error & FLAG_PF_USER, error & FLAG_PF_PRESENT,
      error & FLAG_PF_RDWR, error & FLAG_PF_INSTR_FETCH);
...
extern "C" void irqHandler_9() {
  ...
  ++outstanding_EOIs;
  BlockDevTable::getInstance()->serviceIRQ(9);
...
extern "C" void irqHandler_11() {
  ...
  ++outstanding_EOIs;
  BlockDevTable::getInstance()->serviceIRQ(11);
...
extern "C" void irqHandler_14() {
  ...
  ++outstanding_EOIs;
  BlockDevTable::getInstance()->serviceIRQ(14);
...
extern "C" void irqHandler_15() {
  ...
  ++outstanding_EOIs;
  BlockDevTable::getInstance()->serviceIRQ(15);
...
extern "C" void syscallHandler() {
  ...
  auto ret = Syscall::syscallException(
      currentThread->user_registers_->rax, currentThread->user_registers_->rbx,
      currentThread->user_registers_->rcx, currentThread->user_registers_->rdx,
      currentThread->user_registers_->rsi, currentThread->user_registers_->rdi);
...
extern "C" void errorHandler(size_t num, size_t eip, size_t cs,
  ...
  debug(CPU_ERROR,
        "Instruction Pointer: %zx, Userspace: %d - currentThread: %p %zd"
        ":%s, switch_to_userspace_: %d\n",
        eip, userspace, currentThread,
        currentThread ? currentThread->getTID() : -1UL,
        currentThread ? currentThread->getName() : 0,
        currentThread ? currentThread->switch_to_userspace_ : -1);

  const KernelDebugInfo *deb = kernel_debug_info;
  assert(currentThread &&
         "there should be no fault before there is a current thread");
  assert(currentThread->kernel_registers_ &&
         "every thread needs kernel registers");
  ...
  if (userspace) {
    ...
    assert(currentThread->user_registers_ &&
           (currentThread->user_registers_->cr3 ==
                currentThread->kernel_registers_->cr3 &&
            "User and Kernel CR3 register values differ, this most likely is a "
            "bug!"));
