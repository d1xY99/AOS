...
multiboot_info_t *multi_boot_structure_pointer =
    (multiboot_info_t *)0xDEADDEAD; // must not be in bss segment
static struct multiboot_remainder mbr
    __attribute__((section(".data"))); // must not be in bss segment

extern "C" void parseMultibootHeader() {
  uint32 i;
  multiboot_info_t *mb_infos = *(multiboot_info_t **)VIRTUAL_TO_PHYSICAL_BOOT(
      (pointer)&multi_boot_structure_pointer);
  struct multiboot_remainder &orig_mbr = (struct multiboot_remainder &)(*(
      (struct multiboot_remainder *)VIRTUAL_TO_PHYSICAL_BOOT((pointer)&mbr)));

  if (mb_infos && mb_infos->f_vbe) {
    struct vbe_mode *mode_info =
        (struct vbe_mode *)(uint64)mb_infos->vbe_mode_info;
  ...
  if (mb_infos && mb_infos->f_mods) {
    ...
    for (i = 0; i < mb_infos->mods_count; ++i) {
      ...
      strncpy((char *)(uint64)orig_mbr.module_maps[i].name,
              (const char *)(uint64)mods[i].string, 256);
    }
  ...
  if (mb_infos && mb_infos->f_mmap) {
    ...
    while ((uint64)map <
           (uint64)(mb_infos->mmap_addr + mb_infos->mmap_length)) {
      orig_mbr.memory_maps[i].used = 1;
      orig_mbr.memory_maps[i].start_address =
          ((uint64)map->base_addr_high << 32) | ((uint64)map->base_addr_low);
      orig_mbr.memory_maps[i].end_address =
          orig_mbr.memory_maps[i].start_address +
...
pointer ArchCommon::getFreeKernelMemoryStart() {
  ...
  for (size_t i = 0; i < getNumModules(); ++i)
    free_kernel_memory_start =
        Max(getModuleEndAddress(i), free_kernel_memory_start);
  return ((free_kernel_memory_start - 1) | 0xFFF) + 1;
}

pointer ArchCommon::getFreeKernelMemoryEnd() {
  return KernelHeap::instance()->getKernelBreak();
}
...
size_t ArchCommon::getVESAConsoleHeight() { return mbr.vesa_y_res; }

size_t ArchCommon::getVESAConsoleWidth() { return mbr.vesa_x_res; }
...
size_t ArchCommon::getUsableMemoryRegion(size_t region, pointer &start_address,
                                         pointer &end_address, size_t &type) {
...
Console *ArchCommon::createConsole(size_t count) {
  ...
  debug(THREAD, "Creating console | VESA: %zu\n", haveVESAConsole());
  if (haveVESAConsole())
...
#define PRINT(X) writeLineDebug((const char *)VIRTUAL_TO_PHYSICAL_BOOT(X))
#else
#define PRINT(X)
#endif
...
extern "C" void entry64() {
  PRINT("Reading Multiboot header...\n");
  parseMultibootHeader();
  PRINT("Building kernel page tables...\n");
  initialisePaging();
  PRINT("CR3 loaded...\n");
  asm("mov %%rax, %%cr3"
      :
      : "a"(VIRTUAL_TO_PHYSICAL_BOOT(
          ArchMemory::getRootOfKernelPagingStructure()))
      : "memory");
  PRINT("Switching to kernel stack...\n");
  asm("mov %[stack], %%rsp\n"
      "mov %[stack], %%rbp\n"
      :
      : [stack] "i"(boot_stack + 0x4000)
      : "memory");
  PRINT("Loading 64-bit segments...\n");
...
KernelDebugInfo const *kernel_debug_info = 0;

void ArchCommon::initDebug() {
  for (size_t i = 0; i < getNumModules(); ++i) {
    if (memcmp("AOSDBG1", (char const *)getModuleStartAddress(i), 8) == 0)
      kernel_debug_info =
          new AOSDebugInfo((char const *)getModuleStartAddress(i),
                            (char const *)getModuleEndAddress(i));
  }
  if (!kernel_debug_info)
    kernel_debug_info = new AOSDebugInfo(0, 0);
}

void ArchCommon::idle() { asm volatile("hlt"); }
...
void ArchCommon::drawStat() {
  const char *text =
      "Free pages      F9 MemInfo   F10 Locks   F11 Stacktrace   F12 Threads";
  const char *color =
      "xxxxxxxxxx      xx           xxx         xxx              xxx        ";

  size_t i = 0;
  char itoa_buffer[33];

  if (haveVESAConsole()) {
    FrameBufferConsole *fb_console =
        static_cast<FrameBufferConsole *>(main_console);
    if (fb_console) {
      size_t row = 0;
      ...
      while (text[i]) {
        uint8 state =
            (color[i] == 'x' ? 0x80
                             : 0x08); // 0x80 -> light bakground/dark text, 0x08
      ...
      itoa(FrameAlloc::instance()->getNumFreePages(), itoa_buffer, 10);

      for (size_t j = 0; j < sizeof(itoa_buffer) && itoa_buffer[j] != '\0';
           ++j) {
        fb_console->consoleSetCharacter(row, column + 11 + j, itoa_buffer[j],
                                        0x08);
      }
    }
  } else {
    ...
    for (size_t i = 0; (i < sizeof(itoa_buffer)) && (itoa_buffer[i] != '\0');
         ++i) {
...
void ArchCommon::drawAdditionalStat() {
  ...
  size_t i = 0;
  char itoa_tc_buffer[33];
  char itoa_time_buffer[33];
  ...
  if (haveVESAConsole()) {
    ...
    while (text[i]) {
      fb[(i + 80) * 2] = text[i];
      fb[(i + 80) * 2 + 1] = (char)(color[i] == 'x' ? 0x80 : 0x08);
      i++;
    }

    memset(itoa_tc_buffer, '\0', sizeof(itoa_tc_buffer));
    itoa(Scheduler::instance()->getThreadCount(), itoa_tc_buffer, 10);

    memset(itoa_time_buffer, '\0', sizeof(itoa_time_buffer));
    strncpy(itoa_time_buffer, Scheduler::instance()->getUpTimeString(),
            sizeof(itoa_time_buffer) - 1);

    for (size_t i = 0;
         (i < sizeof(itoa_tc_buffer)) && (itoa_tc_buffer[i] != '\0'); ++i) {
      // 44 is the offset to the "Threads" text
      fb[((i + 80) * 2) + 44] = itoa_tc_buffer[i];
    }

    for (size_t i = 0;
         (i < sizeof(itoa_time_buffer)) && (itoa_time_buffer[i] != '\0'); ++i) {
      // 2 is the offset to the "Uptime" text
      fb[((i + 80) * 2)] = itoa_time_buffer[i];
    }
  }
}
