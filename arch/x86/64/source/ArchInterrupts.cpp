...
void ArchInterrupts::enableTimer() { enableIRQ(0); }
...
void ArchInterrupts::disableTimer() { disableIRQ(0); }
...
void ArchInterrupts::disableKBD() { disableIRQ(1); }

void ArchInterrupts::EndOfInterrupt(uint16 number) { sendEOI(number); }

void ArchInterrupts::enableInterrupts() { asm("sti"); }
...
extern "C" void arch_saveThreadRegisters(uint64 *base, uint64 error) {
  ...
  iregisters =
      (struct interrupt_registers *)(base +
                                     sizeof(struct context_switch_registers) /
                                         sizeof(uint64) +
                                     error);
...
extern "C" void arch_contextSwitch() {
  if (outstanding_EOIs) {
    debug(A_INTERRUPTS,
          "%zu outstanding End-Of-Interrupt signal(s) on context switch. "
          "Probably called yield in the wrong place (e.g. in the scheduler)\n",
          outstanding_EOIs);
    assert(!outstanding_EOIs);
  }
  if (currentThread->switch_to_userspace_) {
    assert(currentThread->holding_lock_list_ == 0 &&
           "Never switch to userspace when holding a lock! Never!");
    assert(currentThread->lock_waiting_on_ == 0 &&
           "How did you even manage to execute code while waiting for a lock?");
  }
  assert(currentThread->isStackCanaryOK() &&
         "Kernel stack corruption detected.");
  ArchThreadRegisters info =
      *currentThreadRegisters; // optimization: local copy produces more
                               // efficient code in this case
  g_tss.rsp0 = info.rsp0;

  // write the fs register for TLS
  uint64 high = info.fs >> 32;
  uint64 low = info.fs & 0xFFFFFFFF;
  ...
  asm volatile("frstor %[fpu]\n"
               ...
               "mov %[rax], %%rax\n"
               "mov %[rbp], %%rbp\n"
               "iretq" ::[fpu] "m"(info.fpu),
               [cr3] "r"(info.cr3), [ss] "m"(info.ss), [rsp] "m"(info.rsp),
               [rflags] "m"(info.rflags), [cs] "m"(info.cs),
               [rip] "m"(info.rip), [rsi] "m"(info.rsi), [rdi] "m"(info.rdi),
               [es] "m"(info.es), [ds] "m"(info.ds), [r8] "m"(info.r8),
               [r9] "m"(info.r9), [r10] "m"(info.r10), [r11] "m"(info.r11),
               [r12] "m"(info.r12), [r13] "m"(info.r13), [r14] "m"(info.r14),
               [r15] "m"(info.r15), [rdx] "m"(info.rdx), [rcx] "m"(info.rcx),
               [rbx] "m"(info.rbx), [rax] "m"(info.rax), [rbp] "m"(info.rbp)
               : "memory");
