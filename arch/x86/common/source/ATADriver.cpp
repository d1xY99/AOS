...
void ATADriver::testIRQ( )
{
  ...
  BlockDevTable::getInstance()->probeIRQ = true;
  readSector( 0, 1, 0 );

  TIMEOUT_CHECK(BlockDevTable::getInstance()->probeIRQ,mode = BD_PIO_NO_IRQ;);
}
...
uint32 ATADriver::addRequest( BlockReq *br )
{
  ...
  if( res != 0 )
  {
    br->setStatus( BlockReq::BD_ERROR );
  ...
  if (currentThread)
  {
    if(interrupt_context)
      ArchInterrupts::enableInterrupts();
    jiffies = 0;
    while (br->getStatus() == BlockReq::BD_QUEUED && jiffies++ < IO_TIMEOUT*10);
    if (jiffies >= IO_TIMEOUT*10)
      TIMEOUT_WARNING();
    if (br->getStatus() == BlockReq::BD_QUEUED)
    {
      ArchInterrupts::disableInterrupts();
      if (br->getStatus() == BlockReq::BD_QUEUED)
      {
...
void ATADriver::nextRequest(BlockReq* br)
{
  if(br->getThread())
...
void ATADriver::serviceIRQ()
{
  ...
  else if( br->getCmd() == BlockReq::BD_WRITE )
  {
    blocks_done++;
    if( blocks_done == br->getNumBlocks() )
    {
