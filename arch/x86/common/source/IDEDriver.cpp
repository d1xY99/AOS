      ...
      else
      {
        ...
        else
        {
          ...
          if (((c4 == 0x14) && (c5 == 0xEB)) || ((c4 == 0x69) && (c5 == 0x96)))
          {
            ...
            // BlockDev *bdv = new
            // BlockDev( drv, 0, drv->getNumSectors(),
            // drv->getSectorSize(), name, true);
            // BlockDevTable::getInstance()->addDevice( bdv );
          ...
          else
          {
            if (c3 != 0)
            {
              if ((c4 == 0x00) && (c5 == 0x00))
              {
                ...
                BlockDev *bdv = new BlockDev(drv, 0, drv->getNumSectors(), drv->getSectorSize(), name,
                                                           true);

                BlockDevTable::getInstance()->addVirtualDevice(bdv);
...
int32 IDEDriver::processMBR(BlockDevDriver* drv, uint32 sector, uint32 SPT, const char *name)
{
    ...
    for (i = 0; i < 4; i++, fp++)
    {
      switch (fp->systid)
      {
        ...
        default:
          ...
          part_name[5] = 0;
          part_num++;
          BlockDev *bdv = new BlockDev(drv, offset, numsec, drv->getSectorSize(), part_name, true);
