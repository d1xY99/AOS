...
{
  paging_root_page_ = FrameAlloc::instance()->allocPPN(PAGE_SIZE);
...
bool ArchMemory::unmapPage(size_t virtual_page)
{
    ...
    FrameAlloc::instance()->freePPN(m.page_ppn);
    ...
    if(empty)
    {
        ...
        FrameAlloc::instance()->freePPN(m.level3_ppn);
    }

    if(empty)
    {
        empty = checkAndRemove<Level12TableEntry>((pointer)m.level1_entry, m.level1_index);
        FrameAlloc::instance()->freePPN(m.level2_ppn);
    }
...
bool ArchMemory::mapPage(size_t virtual_page, size_t physical_page, size_t user_access,
                         size_t writeable, bool executable)
{
    (void)writeable;
    ...
    if(m.level2_ppn == 0)
    {
        m.level2_ppn = FrameAlloc::instance()->allocPPN(PAGE_SIZE);
    ...
    if(m.level3_ppn == 0)
    {
        m.level3_ppn = FrameAlloc::instance()->allocPPN(PAGE_SIZE);
    ...
    if (m.page_ppn == 0)
    {
        ...
        m.level3_entry[m.level3_index].privileged_execute_never = executable ? 0 : 1;
        m.level3_entry[m.level3_index].execute_never = executable ? 0 : 1;
                ...
                if(level2_entry[level2_index].table.entry_descriptor_type == ENTRY_DESCRIPTOR_TABLE)
                {
                    ...
                    for(int level3_index = 0; level3_index < LEVEL3_ENTRIES; level3_index++)
                    {
                        if(level3_entry[level3_index].entry_descriptor_type == ENTRY_DESCRIPTOR_PAGE)
                        {
                            ...
                            FrameAlloc::instance()->freePPN(level3_entry[level3_index].page_address);
                        }
                    }
                    level2_entry[level2_index].table.entry_descriptor_type = 0;
                    FrameAlloc::instance()->freePPN(level2_entry[level2_index].table.table_address);
                }
            }
            level1_entry[level1_index].entry_descriptor_type = 0;
            FrameAlloc::instance()->freePPN(level1_entry[level1_index].table_address);
        }
    }

    FrameAlloc::instance()->freePPN(paging_root_page_);
}
...
const ArchMemoryMapping ArchMemory::resolveMapping(size_t level1_ppn, size_t vpage)
{
    ...
    size_t total_num_pages = FrameAlloc::instance()->getTotalNumPages();
