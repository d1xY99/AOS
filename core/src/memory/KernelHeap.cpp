...
extern KernelDebugInfo const* kernel_debug_info;

KernelHeap kmm;

KernelHeap * KernelHeap::instance_;
size_t KernelHeap::pm_ready_;

KernelHeap* KernelHeap::instance()
{
  if (unlikely(!instance_))
  {
    assert(false && "you can not use KernelHeap::instance before the FrameAlloc is ready!");
  }
  return instance_;
}

KernelHeap::KernelHeap(size_t min_heap_pages, size_t max_heap_pages) :
        tracing_(false), lock_("KMM::lock_"), segments_used_(0), segments_free_(0), approx_memory_free_(0)
{
  ...
  first_ = (HeapSegment*)start_address;
  new ((void*)start_address) HeapSegment(0, 0, min_heap_pages * PAGE_SIZE - sizeof(HeapSegment), false);
  last_ = first_;
  debug(KMM, "KernelHeap::ctor, Heap starts at %zx and initially ends at %zx\n", start_address, start_address + min_heap_pages * PAGE_SIZE);
}

pointer KernelHeap::allocateMemory(size_t requested_size, pointer called_by)
{
...
pointer KernelHeap::private_AllocateMemory(size_t requested_size, pointer called_by)
{
  ...
  // find next free pointer of neccessary size + sizeof(HeapSegment);
  HeapSegment *new_pointer = findFreeSegment(requested_size);

  if (new_pointer == 0)
  {
    unlockKMM();
    kprintfd("KernelHeap::allocateMemory: Not enough Memory left\n");
  ...
  return ((pointer) new_pointer) + sizeof(HeapSegment);
}

bool KernelHeap::freeMemory(pointer virtual_address, pointer called_by)
{
  assert(Thread::currentThreadIsStackCanaryOK() && "Kernel stack corruption detected.");
  ...
  HeapSegment *m_segment = getSegmentFromAddress(virtual_address);
...
pointer KernelHeap::reallocateMemory(pointer virtual_address, size_t new_size, pointer called_by)
{
  assert(Thread::currentThreadIsStackCanaryOK() && "Kernel stack corruption detected.");
  ...
  else
  {
    ...
    if (new_address == 0)
    {
      ...
      kprintfd("KernelHeap::reallocateMemory: Not enough Memory left\n");
      kprintfd("Are we having a memory leak in the kernel??\n");
      kprintfd(
          "This might as well be caused by running too many threads/processes, which partially reside in the kernel.\n");
      assert(false && "Kernel Heap is out of memory");
...
HeapSegment *KernelHeap::getSegmentFromAddress(pointer virtual_address)
{
  HeapSegment *m_segment;
  m_segment = (HeapSegment*) (virtual_address - sizeof(HeapSegment));
...
HeapSegment *KernelHeap::findFreeSegment(size_t requested_size)
{
  debug(KMM, "findFreeSegment: seeking memory block of bytes: %zd \n", requested_size + sizeof(HeapSegment));

  HeapSegment *current = first_;
  while (current != 0)
  {
    debug(KMM, "findFreeSegment: current: %p size: %zd used: %d \n", current, current->getSize() + sizeof(HeapSegment),
          current->getUsed());
  ...
  if(last_->getUsed())
  {
    ...
    HeapSegment* new_segment = new ((void*)ksbrk(sizeof(HeapSegment) + requested_size)) HeapSegment(last_, 0, requested_size, 0);
...
void KernelHeap::fillSegment(HeapSegment *this_one, size_t requested_size, uint32 zero_check)
{
  ...
  if (zero_check)
  {
    for (uint32 i = 0; i < requested_size / 4; ++i)
    {
      if(unlikely(mem[i] != 0))
      {
        ...
        kprintfd("KernelHeap::fillSegment: WARNING: Memory not zero at %p (value=%x)\n", mem + i, mem[i]);
        if(this_one->freed_at_)
        {
          if(kernel_debug_info)
          {
            kprintfd("KernelHeap::freeSegment: The chunk may previously be freed at: ");
  ...
  if (space_left > sizeof(HeapSegment))
  {
    this_one->setSize(requested_size);
    assert(this_one->getSize() == requested_size && "size error");
    assert(this_one->getUsed() == true && "trying to fill an unused segment");

    HeapSegment *new_segment =
        new ((void*) (((pointer) this_one) + sizeof(HeapSegment) + requested_size)) HeapSegment(
            this_one, this_one->next_, space_left - sizeof(HeapSegment), false);
    if (this_one->next_ != 0)
    {
  ...
  debug(KMM, "fillSegment: filled memory block of bytes: %zd \n", this_one->getSize() + sizeof(HeapSegment));
}

void KernelHeap::freeSegment(HeapSegment *this_one)
{
  debug(KMM, "KernelHeap::freeSegment(%p)\n", this_one);
  assert(this_one != 0 && "trying to access a nullpointer");
  assert(this_one->markerOk() && "memory corruption - probably 'write after delete'");

  if (this_one->getUsed() == false)
  {
    kprintfd("KernelHeap::freeSegment: FATAL ERROR\n");
    kprintfd("KernelHeap::freeSegment: tried freeing not used memory block\n");
    if(this_one->freed_at_)
    {
      if(kernel_debug_info)
      {
  ...
  if (this_one->prev_ != 0)
  {
    ...
    if (this_one->prev_->getUsed() == false)
    {
      ...
      HeapSegment *previous_one = this_one->prev_;
      ...
      debug(KMM, "freeSegment: previous_one: %p size: %zd used: %d\n", previous_one,
            previous_one->getSize() + sizeof(HeapSegment), previous_one->getUsed());
  ...
  memset((void*) ((size_t) this_one + sizeof(HeapSegment)), 0, this_one->getSize()); // ease debugging
  ...
  if(this_one == last_)
  {
    if(this_one != first_)
    {
      ...
      if((size_t)this_one > base_break_ + reserved_min_)
      {
        ...
        ksbrk(-(this_one->getSize() + sizeof(HeapSegment)));
      }
      else if((size_t)this_one + sizeof(HeapSegment) + this_one->getSize() <= base_break_ + reserved_min_)
      {
        // Case 2
      ...
      else
      {
        ...
        size_t segment_size = (base_break_ + reserved_min_) - ((size_t)this_one + sizeof(HeapSegment));
...

  {
    ...
    while (current != 0)
    {
      debug(KMM, "freeSegment: current: %p prev: %p next: %p size: %zd used: %d\n", current, current->prev_,
            current->next_, current->getSize() + sizeof(HeapSegment), current->getUsed());
      assert(current->markerOk() && "memory corruption - probably 'write after delete'");
      current = current->next_;
    }
...
bool KernelHeap::mergeWithFollowingFreeSegment(HeapSegment *this_one)
{
  assert(this_one != 0 && "trying to access a nullpointer");
  assert(this_one->markerOk() && "memory corruption - probably 'write after delete'");
  ...
  if (this_one->next_ != 0)
  {
    ...
    {
      HeapSegment *next_one = this_one->next_;
      ...
      memset((void*) next_one, 0, sizeof(HeapSegment));
...
pointer KernelHeap::ksbrk(ssize_t size)
{
  ...
  assert(DYNAMIC_KMM && "ksbrk should only be called if DYNAMIC_KMM is 1 - not in baseline AOS");
  if(size != 0)
  {
    ...
    if(size > 0)
    {
      ...
      while(cur_top_vpn != new_top_vpn)
      {
        debug(KMM, "%zx != %zx\n", cur_top_vpn, new_top_vpn);
        cur_top_vpn++;
        assert(pm_ready_ && "Kernel Heap should not be used before FrameAlloc is ready");
        size_t new_page = FrameAlloc::instance()->allocPPN();
        if(unlikely(new_page == 0))
        {
          kprintfd("KernelHeap::freeSegment: FATAL ERROR\n");
          kprintfd("KernelHeap::freeSegment: no more physical memory\n");
...
Thread* KernelHeap::KMMLockHeldBy()
{
  return lock_.heldBy();
}

void KernelHeap::lockKMM()
{
  assert((!(system_state == RUNNING) || FrameAlloc::instance()->heldBy() != currentThread) && "You're abusing the FrameAlloc lock");
  lock_.acquire(getCalledBefore(1));
}

void KernelHeap::unlockKMM()
{
  assert((!(system_state == RUNNING) || FrameAlloc::instance()->heldBy() != currentThread) && "You're abusing the FrameAlloc lock");
  lock_.release(getCalledBefore(1));
}

SpinLock& KernelHeap::getKMMLock()
{
  return lock_;
}

size_t KernelHeap::getUsedKernelMemory(bool show_allocs = false) {
    HeapSegment *current = first_;
...
void KernelHeap::startTracing() {
    tracing_ = true;
}

void KernelHeap::stopTracing() {
    tracing_ = false;
}

pointer KernelHeap::getKernelBreak() const {
  return kernel_break_;
}
