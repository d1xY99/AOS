...
TextConsole::TextConsole(uint32 num_terminals)
    : Console(num_terminals, "TextConsoleWorker") {
...
uint32 TextConsole::consoleGetNumRows() const { return 25; }

uint32 TextConsole::consoleGetNumColumns() const { return 80; }
...
uint32 TextConsole::consoleSetCharacter(uint32 const &row, uint32 const &column,
...
void TextConsole::consoleScrollUp(uint8 const &state) {
  ...
  for (size_t i = 0; i < consoleGetNumColumns(); i++) {
    ...
    fb[(i + (consoleGetNumRows() - 1) * consoleGetNumColumns()) * 2 + 1] =
        state;
  }
}
