...
FrameBufferConsole::FrameBufferConsole(uint32 num_terminals)
  ...
  for (i = 0; i < num_terminals; ++i) {
    ...
    Terminal *term = new Terminal(term_name, this, consoleGetNumColumns(),
                                  consoleGetNumRows() - 16);
...
void FrameBufferConsole::consoleClearScreen() {
  memset((void *)ArchCommon::getVESAConsoleLFBPtr(), 0,
         x_res_ * y_res_ * bytes_per_pixel_);
}

uint32 FrameBufferConsole::consoleGetNumRows() const { return y_res_ / 16; }

uint32 FrameBufferConsole::consoleGetNumColumns() const { return x_res_ / 8; }

extern uint8 fontdata_sun8x16[];

void FrameBufferConsole::setPixel(uint32 x, uint32 y, uint8 r, uint8 g,
                                  uint8 b) {
...
uint32 FrameBufferConsole::consoleSetCharacter(uint32 const &row,
                                               uint32 const &column,
                                               uint8 const &character,
                                               uint8 const &state) {
  CONSOLECOLOR fg, bg;
...
void FrameBufferConsole::consoleScrollUp(uint8 const &state) {
  ...
  memmove((void *)fb,
          (void *)(fb + (consoleGetNumColumns() * bytes_per_pixel_ * 8 * 16)),
          (consoleGetNumRows() - 1) * consoleGetNumColumns() *
              bytes_per_pixel_ * 8 * 16);

  for (uint32 index = ((consoleGetNumRows() - 1) * consoleGetNumColumns() *
                       bytes_per_pixel_ * 8 * 16) /
                      2;
...
void FrameBufferConsole::colorsFromState(uint8 const &state, CONSOLECOLOR &fg,
                                         CONSOLECOLOR &bg) {
