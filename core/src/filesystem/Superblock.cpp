...
Superblock::Superblock(FsTypeReg* fs_type, size_t s_dev) :
...
FsTypeReg* Superblock::getFSType()
{
  return (FsTypeReg*) s_type_;
}
...
int Superblock::fileOpened(File* file)
{
    ...
    if(aostl::find(s_files_.begin(), s_files_.end(), file) != s_files_.end())
    {
        return -1;
    }
