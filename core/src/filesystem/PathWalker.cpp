...
int32 PathWalker::pathWalk(const char* pathname, FsContext* fs_info, Path& out, Path* parent_dir)
{
  assert(fs_info);
...
aostl::string PathWalker::pathPrefix(const aostl::string& path)
{
    ...
    aostl::string retval = path.substr(0, prefix_len);
    debug(PATHWALKER, "pathPrefix: %s -> %s\n", path.c_str(), retval.c_str());
    return retval;
}

aostl::string PathWalker::lastPathSegment(const aostl::string& path, bool ignore_separator_at_end)
{
    if(path.length() == 0)
        return path;
    ...
    {
        ssize_t prefix_len = path.find_last_of("/");
        aostl::string retval = path.substr(prefix_len+1, path.length() - prefix_len);
        debug(PATHWALKER, "lastPathSegment: %s -> %s\n", path.c_str(), retval.c_str());
        return retval;
    }
    else
    {
        aostl::string tmp_path = path.substr(0, path.find_last_not_of("/") + 1);
        ssize_t prefix_len = tmp_path.find_last_of("/");
        aostl::string retval = tmp_path.substr(prefix_len+1, tmp_path.length() - prefix_len);
        debug(PATHWALKER, "lastPathSegment: %s -> %s\n", path.c_str(), retval.c_str());
        return retval;
    }
}
