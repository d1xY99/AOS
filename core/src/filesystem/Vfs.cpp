...
int32 Vfs::registerFileSystem(FsTypeReg *file_system_type)
{
...
int32 Vfs::unregisterFileSystem(FsTypeReg *file_system_type)
{
  assert(file_system_type != 0);

  const char *fs_name = file_system_type->getFSName();
  for (FsTypeReg* fst : file_system_types_)
  {
...
FsTypeReg *Vfs::getFsType(const char* fs_name)
{
  assert(fs_name);
...
MountPoint *Vfs::getVfsMount(const Dentry* dentry, bool is_root)
{
  assert(dentry);

  if (is_root == false)
  {
    for (MountPoint* mnt : mounts_)
    {
...
FsContext *Vfs::rootMount(const char *fs_name, uint32 /*flags*/)
{
  FsTypeReg *fst = getFsType(fs_name);
  if(!fst)
  {
  ...
  MountPoint *root_mount = new MountPoint(0, root, root, super, 0);

  mounts_.push_back(root_mount);
  superblocks_.push_back(super);

  // fs_info initialize
  FsContext *fs_info = new FsContext();
...
int32 Vfs::mount(const char* dev_name, const char* dir_name, const char* fs_name, uint32 /*flags*/)
{
  ...
  FsContext *fs_info = currentThread->getWorkingDirInfo();
  assert(fs_info);

  uint32_t dev = -1;
  if(fst->getFSFlags() & FS_REQUIRES_DEV)
  {
      BlockDev* bddev = BlockDevTable::getInstance()->getDeviceByName(dev_name);
      if (!bddev)
      {
  ...
  MountPoint* new_mount = new MountPoint(mountpoint_path.mnt_, mountpoint_path.dentry_, root, super, 0);
...
int32 Vfs::rootUmount()
{
  if (superblocks_.size() == 0)
  {
    return -1;
  }
  MountPoint *root_vfs_mount = mounts_.at(0);
...
int32 Vfs::umount(const char* dir_name, uint32 /*flags*/)
{
  FsContext *fs_info = currentThread->getWorkingDirInfo();
  if (dir_name == 0)
    return -1;
