...
Path::Path(Dentry* dentry, MountPoint* mnt) :
...
#ifndef AOS_MKIMG
    MountPoint* vfs_mount = vfs.getVfsMount(child_dentry);
    if (vfs_mount != 0)
    {
...
aostl::string Path::getAbsolutePath(const Path* global_root) const
{
    if(isGlobalRoot(global_root))
    {
        return "/";
