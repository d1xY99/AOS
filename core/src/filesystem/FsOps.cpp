...
FileDescriptor* FsOps::getFileDescriptor(uint32 fd)
{
  return global_fd_list.getFileDescriptor(fd);
}

#ifndef AOS_MKIMG
static FileDescriptor *resolveProcessFileDescriptor(uint32 fd) {
  ...
  FileDescriptor *fd_ptr =
      user_thread->process_->fileDescriptorTable().getKernelDescriptor(
          static_cast<int32>(fd));
  debug(VFSSYSCALL, "resolveProcessFileDescriptor: fd %u -> %p\n", fd,
        fd_ptr);
  return fd_ptr;
}
#endif
...
int32 FsOps::mkdir(const char* pathname, int32)
{
  ...
  {
    aostl::string new_dir_name = PathWalker::lastPathSegment(pathname, true);
...
Dirent* FsOps::readdir(const char* pathname, char* buffer, size_t size)
{
  FsContext *fs_info = getcwd();
...
int32 FsOps::chdir(const char* pathname)
{
...
int32 FsOps::rm(const char* pathname)
{
...
int32 FsOps::rmdir(const char* pathname)
{
  FsContext *fs_info = getcwd();
...
int32 FsOps::open(const char* pathname, uint32 flag)
{
  if(!pathname)
  {
  ...
  else if ((path_walk_status == PW_ENOTFOUND) && (flag & O_CREAT))
  {
    ...
    aostl::string new_dentry_name = PathWalker::lastPathSegment(pathname);
...
int32 FsOps::read(uint32 fd, char* buffer, uint32 count)
{
  FileDescriptor* file_descriptor = nullptr;
#ifndef AOS_MKIMG
  file_descriptor = resolveProcessFileDescriptor(fd);
  if (!file_descriptor)
#endif
...
int32 FsOps::write(uint32 fd, const char *buffer, uint32 count)
{
  FileDescriptor* file_descriptor = nullptr;
#ifndef AOS_MKIMG
  file_descriptor = resolveProcessFileDescriptor(fd);
  ...
  if (file_descriptor == 0)
  {
    ...
    debug(VFSSYSCALL, "(write) lookup failed for fd %u\n", fd);
    return -1;
  }
...
l_off_t FsOps::lseek(uint32 fd, l_off_t offset, uint8 origin)
{
  FileDescriptor* file_descriptor = nullptr;
#ifndef AOS_MKIMG
  file_descriptor = resolveProcessFileDescriptor(fd);
...
#ifndef AOS_MKIMG
int32 FsOps::mount(const char *device_name, const char *dir_name, const char *file_system_name, int32 flag)
{
  FsTypeReg* type = vfs.getFsType(file_system_name);
  if (!type)
  {
...
int32 FsOps::umount(const char *dir_name, int32 flag)
{
  return vfs.umount(dir_name, flag);
}
#endif

uint32 FsOps::getFileSize(uint32 fd)
{
  FileDescriptor* file_descriptor = getFileDescriptor(fd);
