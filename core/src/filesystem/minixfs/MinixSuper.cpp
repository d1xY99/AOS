...
MinixSuper::MinixSuper(MinixFsReg* fs_type, size_t s_dev, uint64 offset) :
    Superblock(fs_type, s_dev), superblock_(this), offset_(offset)
{
  ...
  storage_manager_ = new MinixZoneAlloc(bm_buffer, s_num_inode_bm_blocks_, s_num_zone_bm_blocks_, s_num_inodes_,
                                             s_zones_);
...
void MinixSuper::initInodes()
{
  debug(M_SB, "init Inodes\n");
  MinixNode *root_inode = getInode(1);
...
MinixNode* MinixSuper::getInode(uint16 i_num, bool &is_already_loaded)
{
  MinixNode* tmp = (MinixNode*) all_inodes_set_[i_num];
  if (tmp)
  {
...
MinixNode* MinixSuper::getInode(uint16 i_num)
{
  ...
  inode = new MinixNode(this, ((uint16*) ibuffer)[0], ((uint32*) ibuffer)[1 + V3_OFFSET],
                           ((uint16*) ibuffer)[V3_OFFSET], i_zones, i_num);
  debug(M_SB, "getInode:: returned creating Inode\n");
  return inode;
}

MinixSuper::~MinixSuper()
{
...
Inode* MinixSuper::createInode(uint32 type)
{
  uint16 mode = 0x01ff;
  if (type == I_FILE)
    mode |= 0x8000;
  ...
  Inode *inode = new MinixNode(this, mode, 0, 0, zones, i_num);
...
int32 MinixSuper::readInode(Inode* inode)
{
  assert(inode);
  MinixNode *minix_inode = (MinixNode *) inode;
  assert(aostl::find(all_inodes_.begin(), all_inodes_.end(), inode) != all_inodes_.end());
  ...
  MinixZone *to_delete_i_zones = minix_inode->i_zones_;
  minix_inode->i_zones_ = new MinixZone(this, i_zones);
...
void MinixSuper::writeInode(Inode* inode)
{
  assert(inode);
  assert(aostl::find(all_inodes_.begin(), all_inodes_.end(), inode) != all_inodes_.end());
  //flush zones
...
void MinixSuper::all_inodes_add_inode(Inode* inode)
{
  all_inodes_.push_back(inode);
  all_inodes_set_[((MinixNode*) inode)->i_num_] = inode;
}

void MinixSuper::all_inodes_remove_inode(Inode* inode)
{
  all_inodes_.remove(inode);
  all_inodes_set_.erase(((MinixNode*) inode)->i_num_);
}

void MinixSuper::deleteInode(Inode* inode)
{
  assert(inode->getDentrys().size() == 0);
  assert(aostl::find(used_inodes_.begin(), used_inodes_.end(), inode) == used_inodes_.end());
...
uint16 MinixSuper::allocateZone()
{
  debug(M_ZONE, "MinixSuper allocateZone>\n");
  ...
  debug(M_ZONE, "MinixSuper allocateZone> returning %d\n", ret);
  return ret;
}

void MinixSuper::readZone(uint16 zone, char* buffer)
{
  assert(buffer);
  readBlocks(zone, ZONE_SIZE / BLOCK_SIZE, buffer);
}

void MinixSuper::readBlocks(uint16 block, uint32 num_blocks, char* buffer)
{
  assert(buffer);
#ifdef AOS_MKIMG
...
#else
  BlockDev* bdvd = BlockDevTable::getInstance()->getDeviceByNumber(s_dev_);
  ...
  assert(bdvd->readData(block * block_size, read_size, buffer) == (ssize_t)read_size &&
      "Disk read failed in MinixSuper::readBlocks()");
#endif
}

void MinixSuper::writeZone(uint16 zone, char* buffer)
{
  writeBlocks(zone, ZONE_SIZE / BLOCK_SIZE, buffer);
}

void MinixSuper::writeBlocks(uint16 block, uint32 num_blocks, char* buffer)
{
#ifdef AOS_MKIMG
  ...
  assert(bdvd->writeData(block * block_size, write_size, buffer) == (ssize_t)write_size &&
      "Disk write failed in MinixSuper::writeBlocks()");
#endif
}

int32 MinixSuper::readBytes(uint32 block, uint32 offset, uint32 size, char* buffer)
{
...
int32 MinixSuper::writeBytes(uint32 block, uint32 offset, uint32 size, char* buffer)
{
  assert(offset+size <= BLOCK_SIZE);
...
void MinixSuper::freeZone(uint16 index)
{
