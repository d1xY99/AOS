#include "MinixZoneAlloc.h"
#include "MinixSuper.h"
#include <assert.h>
#include "kprintf.h"

MinixZoneAlloc::MinixZoneAlloc(char *bm_buffer, uint16 num_inode_bm_blocks, uint16 num_zone_bm_blocks,
                                         uint16 num_inodes, uint16 num_zones) :
    ZoneAllocBase(num_inodes, num_zones)
{
...
MinixZoneAlloc::~MinixZoneAlloc()
{
  debug(M_STORAGE_MANAGER, "Destructor: destroyed\n");
}

bool MinixZoneAlloc::isInodeSet(size_t index)
{
  assert(index < inode_bitmap_.getSize() && "MinixZoneAlloc::isInodeSet called with bad index number");
  return inode_bitmap_.getBit(index);
}

uint32 MinixZoneAlloc::getNumUsedInodes()
{
  return inode_bitmap_.getNumBitsSet();
}

size_t MinixZoneAlloc::allocZone()
{
...
size_t MinixZoneAlloc::allocInode()
{
...
void MinixZoneAlloc::printBitmap()
{
