...
MinixNode::MinixNode(Superblock *super_block, uint32 inode_type) :
...
MinixNode::MinixNode(Superblock *super_block, uint16 i_mode, uint32 i_size, uint16 i_nlinks, uint32* i_zones,
                           uint32 i_num) :
    Inode(super_block, 0), i_zones_(new MinixZone((MinixSuper*) super_block, i_zones)), i_num_(i_num),
    children_loaded_(false)
{
  i_size_ = i_size;
  i_nlink_ = i_nlinks;
...
int32 MinixNode::readData(uint32 offset, uint32 size, char *buffer)
{
  assert(buffer);
  ...
  for (uint32 zone = start_zone; zone < start_zone + num_zones; zone++)
  {
    ...
    ((MinixSuper *) superblock_)->readZone(i_zones_->getZone(zone), rbuffer);
...
int32 MinixNode::writeData(uint32 offset, uint32 size, const char *buffer)
{
  debug(M_INODE, "MinixNode writeData> offset: %d, size: %d, i_size_: %d\n", offset, size, i_size_);
  ...
  if ((size + offset) > i_size_)
  {
    ...
    {
      MinixSuper* sb = (MinixSuper*) superblock_;
  ...
  if (offset > i_size_)
  {
    ...
    if (zone_size_offset)
    {
      ...
      ((MinixSuper *) superblock_)->writeZone(last_used_zone, fill_buffer);
    }
    ++last_used_zone;
  ...
  for (uint32 zone_index = 0; zone_index < num_zones; zone_index++)
  {
    ...
    ((MinixSuper *) superblock_)->writeZone(i_zones_->getZone(zone_index + zone), wbuffer);
    wbuffer += ZONE_SIZE;
  }
...
int32 MinixNode::truncate(size_t length)
{
  if (i_type_ != I_FILE)
  {
    return -1;
  ...
  uint32 new_size = static_cast<uint32>(length);
  if (new_size == i_size_)
  {
    return 0;
  }
  ...
  uint32 old_size = i_size_;
  size_t remaining = new_size - old_size;
  char zero_buffer[ZONE_SIZE];
  ...
  while (remaining > 0)
  {
    ...
    int32 written =
        writeData(old_size, static_cast<uint32>(chunk), zero_buffer);
    if (written <= 0)
    {
      return -1;
    }
    old_size += static_cast<uint32>(written);
...
int32 MinixNode::mknod(Dentry *dentry)
{
  Inode::mknod(dentry);
  debug(M_INODE, "mknod: dentry: %p, i_type_: %x\n", dentry, i_type_);

  ((MinixNode *) dentry->getParent()->getInode())->writeDentry(0, i_num_, dentry->getName());
  return 0;
}

int32 MinixNode::mkfile(Dentry *dentry)
{
  Inode::mkfile(dentry);
...
int32 MinixNode::mkdir(Dentry *dentry)
{
  Inode::mkdir(dentry);
  debug(M_INODE, "mkdir: dentry: %p (%s)\n", dentry, dentry->getName());

  MinixNode* parent_inode = ((MinixNode *) dentry->getParent()->getInode());
...
void MinixNode::writeDentry(uint32 dest_i_num, uint32 src_i_num, const char* name)
{
...
int32 MinixNode::link(Dentry* dentry)
{
...
int32 MinixNode::unlink(Dentry* dentry)
{
    ...
    ((MinixNode *) dentry->getParent()->getInode())->writeDentry(i_num_, 0, "");

    return 0;
}

int32 MinixNode::rmdir(Dentry* dentry)
{
  ...
  MinixNode* parent_inode = static_cast<MinixNode*>(dentry->getParent()->getInode());
...
Dentry* MinixNode::lookup(const char* name)
{
  if(i_type_ != I_DIR)
  {
    return nullptr;
  ...
  else
  {
    ...
    if (((MinixNode *) dentry_update->getInode())->i_type_ == I_DIR)
    {
      ((MinixNode *) dentry_update->getInode())->loadChildren();
    }
    return dentry_update;
  }
}

void MinixNode::loadChildren()
{
  if (children_loaded_)
  {
  ...
  for (uint32 zone = 0; zone < i_zones_->getNumZones(); zone++)
  {
    ...
    for (uint32 curr_dentry = 0; curr_dentry < BLOCK_SIZE; curr_dentry += INODE_SIZE)
    {
      ...
      if (inode_index)
      {
        ...
        MinixNode* inode = ((MinixSuper *) superblock_)->getInode(inode_index, is_already_loaded);

        if (!inode)
        {
          kprintfd("MinixNode::loadChildren: inode nr. %d not set in bitmap, but occurs in directory-entry; "
        ...
        {
          ((MinixSuper *) superblock_)->all_inodes_add_inode(inode);
        }
      }
    }
  }
