...
MinixZone::MinixZone(MinixSuper *superblock, uint32 *zones)
{
...
uint32 MinixZone::getZone(uint32 index)
{
...
void MinixZone::setZone(uint32 index, uint32 zone)
{
  debug(M_ZONE, "MinixZone::setZone> index: %d, zone: %d\n", index, zone);
  if (index < 7)
  {
...
void MinixZone::addZone(uint32 zone)
{
  setZone(num_zones_, zone);
}

void MinixZone::flush(uint32 i_num)
{
  debug(M_ZONE, "MinixZone::flush i_num : %d; %p\n", i_num, this);
  ...
  debug(M_ZONE, "MinixZone::flush direct written\n");
  if (direct_zones_[7])
  {
    char ind_buffer[ZONE_SIZE];
    debug(M_ZONE, "MinixZone::flush writing indirect\n");
