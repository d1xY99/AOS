...
Dentry::Dentry(Inode* inode, Dentry* parent, const aostl::string& name) :
...
int32 Dentry::childRemove(Dentry *child_dentry)
{
  debug(DENTRY, "Dentry childRemove d_child_ included: %d\n",
        aostl::find(d_child_.begin(), d_child_.end(), child_dentry) != d_child_.end());
  assert(child_dentry != 0);
...
{
  if (dentry == 0 || aostl::find(d_child_.begin(), d_child_.end(), dentry) != d_child_.end())
    return -1;
