...
BlockDev::BlockDev(BlockDevDriver * driver, uint32 offset, uint32 num_sectors, uint32 sector_size,
...
void BlockDev::addRequest(BlockReq * command)
{
  command->setResult(5);
  switch (command->getCmd())
  {
    case BlockReq::BD_GET_BLK_SIZE:
      command->setResult(block_size_);
      command->setStatus(BlockReq::BD_DONE);
      break;
    case BlockReq::BD_GET_NUM_BLOCKS:
      command->setResult(getNumBlocks());
      command->setStatus(BlockReq::BD_DONE);
      break;
    case BlockReq::BD_READ:
    case BlockReq::BD_WRITE:
...
int32 BlockDev::readData(uint32 offset, uint32 size, char *buffer)
{
  assert(buffer);
  ...
  BlockReq bd(dev_number_, BlockReq::BD_READ, blockoffset, blocks2read, buffer);
  addRequest(&bd);
  ...
  if (driver_->irq != 0)
  {
    ...
    while (bd.getStatus() == BlockReq::BD_QUEUED && jiffies++ < IO_TIMEOUT)
  ...
  if (bd.getStatus() != BlockReq::BD_DONE)
  {
    return -1;
  }
  return size;
...
int32 BlockDev::writeData(uint32 offset, uint32 size, char *buffer)
{
  ...
  BlockReq bd(dev_number_, BlockReq::BD_WRITE, blockoffset, blocks2write, buffer);
  addRequest(&bd);
...
void BlockDev::setPartitionType(uint8 part_type)
{
  partition_type_ = part_type;
}

uint8 BlockDev::getPartitionType(void) const
{
