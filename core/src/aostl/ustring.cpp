...
{
    const_iterator found = ::aostl::find (iat(pos), end(), c);
...
string::pos_type string::find (const string& s, pos_type pos) const noexcept
{
    ...
    for (; i < end() && (i = ::aostl::find (i, end(), endchar)) < end(); i += skip)
...
/*void string::read (istream& is)
{
    ...
    if (!is.verify_remaining ("read", "aostl::string", szsz)) return;
    is.read (szbuf + 1, szsz);
    n = *utf8in(szbuf);
    if (!is.verify_remaining ("read", "aostl::string", n)) return;
    resize (n);
    is.read (data(), size());
}
...
void string::write (ostream& os) const
{
    ...
    *szout = sz;
    ...
    if (!os.verify_remaining ("write", "aostl::string", szsz + sz)) return;
    os.write (szbuf, szsz);
    os.write (cdata(), sz);
}*/
