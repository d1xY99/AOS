...
KernelDebugInfo::KernelDebugInfo(char const *stab_start, char const *stab_end, char const *stab_str) :
...
void KernelDebugInfo::initialiseSymbolTable()
{
...
bool KernelDebugInfo::tryPasteOoperator(const char *& input, char *& buffer, size_t& size) const
{
  if (!input || !buffer)
    return false;
...
ssize_t KernelDebugInfo::getFunctionLine(pointer start, pointer offset) const
{
  ssize_t line = -1;
  aostl::map<size_t, StabEntry const *>::const_iterator it = function_symbols_.find(start);
...
void KernelDebugInfo::getCallNameAndLine(pointer address, const char*& mangled_name, ssize_t &line) const
{
  mangled_name = 0;
  line = 0;
  ...
  aostl::map<size_t, StabEntry const *>::const_iterator it;
...
void KernelDebugInfo::printCallInformation(pointer address) const
{
...
pointer KernelDebugInfo::getFunctionName(pointer address, char function_name[], size_t size) const
{
...
int KernelDebugInfo::readNumber(const char *& input) const
{
  if (!input)
    return -1;
...
void KernelDebugInfo::pasteTypename(const char *& input, char *& buffer, size_t& size) const
{
...
void KernelDebugInfo::pasteArguments(const char *& input, char *& buffer, char delimiter, size_t& size) const
{
  if (!input || !buffer)
    return;
...
size_t KernelDebugInfo::putChar2Buffer(char*& buffer, char c, size_t& size) const
{
  if (size <= 1)
    return -1;
  *buffer++ = c;
...
void KernelDebugInfo::demangleName(const char* name, char *buffer, size_t size) const
{
