...
class HeapSegment
{
  public:
    /**
     * Constructor
     * @param *prev Pointer to the previous HeapSegment in the list
     * @param *next Pointer to the next HeapSegment in the list
     * @param size Number of Bytes the described segment is large
     *        (this + sizeof(HeapSegment) + size is usually the start of the next segment)
     * @param used describes if the segment is allocated or free
     */
    HeapSegment(HeapSegment *prev, HeapSegment *next, size_t size, bool used) :
    ...
    HeapSegment *next_; // = NULL;
    HeapSegment *prev_; // = NULL;
...
class KernelHeap
{
  public:
     ...
     * searches the HeapSegment-List for a free segment with size >= requested_size
  ...
  protected:
    friend class FrameAlloc;

    KernelHeap(size_t min_heap_pages, size_t max_heap_pages);

    static size_t pm_ready_;

    static KernelHeap *instance_;

  private:
    HeapSegment *findFreeSegment(size_t requested_size);
     ...
     */
    void fillSegment(HeapSegment *this_one, size_t size, uint32 zero_check = 1);

    void freeSegment(HeapSegment *this_one);
    HeapSegment *getSegmentFromAddress(pointer virtual_address);
    bool mergeWithFollowingFreeSegment(HeapSegment *this_one);
    ...
    HeapSegment* first_; //first_ must _never_ be NULL
    HeapSegment* last_;
    pointer base_break_;
