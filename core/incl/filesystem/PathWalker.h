...
class PathWalker
{
  public:
    ...
    static int32 pathWalk(const char* pathname, FsContext* fs_info, Path& out, Path* parent_dir = nullptr);

    static aostl::string pathPrefix(const aostl::string& path);
    static aostl::string lastPathSegment(const aostl::string& path, bool ignore_separator_at_end = false);
