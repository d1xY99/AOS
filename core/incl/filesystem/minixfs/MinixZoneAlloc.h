...
class MinixZoneAlloc : public ZoneAllocBase
{
  public:
     ...
     */
    MinixZoneAlloc(char *bm_buffer, uint16 num_inode_bm_blocks, uint16 num_zone_bm_blocks, uint16 num_inodes,
                        uint16 num_zones);

    virtual ~MinixZoneAlloc();
    ...
    void flush(MinixSuper *superblock);
    void printBitmap();
