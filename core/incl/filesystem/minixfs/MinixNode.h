...
class MinixNode : public Inode
{
    friend class MinixSuper;
  ...
  public:
     ...
     */
    MinixNode(Superblock *super_block, uint32 inode_type);
     ...
     */
    MinixNode(Superblock *super_block, uint16 i_mode, uint32 i_size, uint16 i_nlinks, uint32* i_zones, uint32 i_num);
    virtual ~MinixNode();
    ...
    /**
     * truncate or extend the inode to the given length
     * @param length new size in bytes
     * @return 0 on success
     */
    virtual int32 truncate(size_t length);
