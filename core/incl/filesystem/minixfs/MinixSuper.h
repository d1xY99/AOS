...
class MinixSuper : public Superblock
{
  public:
    friend class MinixNode;
    friend class MinixZone;
    friend class MinixZoneAlloc;

    MinixSuper(MinixFsReg* fs_type, size_t s_dev, uint64 offset);
    virtual ~MinixSuper();
  ...
  private:
    ...
    MinixZoneAlloc* storage_manager_;


    aostl::map<uint32, Inode*> all_inodes_set_;
