...
{
  protected:
    aostl::list<Superblock*> superblocks_;
    aostl::list<MountPoint*> mounts_;
    aostl::list<FsTypeReg*> file_system_types_;
  ...
  public:
     ...
     */
    int32 registerFileSystem(FsTypeReg *file_system_type);
     ...
     * @return 0 on success
     */
    int32 unregisterFileSystem(FsTypeReg *file_system_type);
     ...
     * descriptors, and returns a pointer to the corresponding FsTypeReg
     * object, if is present.
     * @param fs_name the name of the filesystem
     * @return the file system type
     */
    FsTypeReg *getFsType(const char* fs_name);

    /**
     * found the MountPoint from mounts_ list with given dentry.
     ...
     */
    MountPoint *getVfsMount(const Dentry* dentry, bool is_root = false);
     ...
     * @return On success, zero is returned. On error, -1 is returned.
     */
    FsContext *rootMount(const char* fs_name, uint32 flags);
