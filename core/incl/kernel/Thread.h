...
#include "filesystem/FsContext.h"
#include "types.h"
...
public:
  static const char *threadStatePrintable[5];
   ...
   */
  Thread(FsContext *working_dir, aostl::string name, Thread::TYPE type,
         size_t tid = 0);
   ...
   * DO Not use new / delete in this Method, as it sometimes called from an
  ...
  FsContext *getWorkingDirInfo();

  void setWorkingDirInfo(FsContext *working_dir);

  /**
   * Prints a backtrace (i.e. the call stack) to the debug output.
   * @param use_stored_thread_info determines whether to use the stored or the
  ...
  /**
   * A single chained list containing all the locks held by the thread at the
   * moment. This list is not locked. It may only be accessed by the thread
   * himself, or by other threads in case they can ENSURE that this thread is
   * not able to run at this moment. Changing the list has to be done atomic,
   * else it cannot be ensured that the list is valid at any moment!
   */
