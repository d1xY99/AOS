...
#define SHELL_DIVIDER                                                          \
  "--------------------------------------------------------------------------" \
  "------"
#define SHELL_CMD_LINE_PREFIX "AOS: /> "
...
char *findOccurrence(char *file_list, char *user_input) {
  ...
  while (occurrence != NULL && occurrence != file_list &&
         *(occurrence - 1) != '\n') {
...
void clearLine() {
  char backslashes[] = {
      [0 ... sizeof(SHELL_CMD_LINE_PREFIX) + SHELL_BUFFER_SIZE - 1] = '\b'};
...
int splitCommandAndArguments() {
  ...
  while (first_space && first_space + 1 < buffer + SHELL_BUFFER_SIZE &&
         arg_counter < SHELL_MAX_ARGS) {
    char *next_space = strstr(first_space + 1, " ");
    size_t arg_length = !next_space
                            ? strlen(first_space + 1)
                            : (size_t)next_space - ((size_t)first_space + 1);
...
void readCommand() {
  ...
  while (keep_reading && counter < SHELL_BUFFER_SIZE) {
    ...
    case '\t': {
      ...
      temp_buffer[count] = 0;
      tab_cursor = occurrence + 1 >= dir_content + SHELL_DIR_CONTENT_SIZE
                       ? dir_content
                       : occurrence + 1;
      break;
    }
...
void handleCommand() {
  ...
  if (strcmp(command, "ls") == 0 || strcmp(command, "l") == 0) {
    __syscall(sc_pseudols, (size_t)(arg_counter > 0 ? args[0] : "."), 0, 0, 0,
              0);
    return;
  }
  ...
  memcpy(executable + strlen(SHELL_EXECUTABLE_PREFIX), command,
         strlen(command) + 1);
...
int main(int argc, char *argv[]) {
  printf("\n%s\n...multithreading our way out of this mess\n"
         "\n   \xc2\xa9 Amir Basic - All Rights Reserved\n"
         "\nShell Starting...\n",
         aos);
  printAvailableCommands();
  __syscall(sc_pseudols, (size_t)SHELL_EXECUTABLE_PREFIX, (size_t)dir_content,
            sizeof(dir_content), 0, 0);
  while (running) {
    readCommand();
    handleCommand();
  }
